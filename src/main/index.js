import {
  app, BrowserWindow, WebContentsView, dialog, Menu, ipcMain,
...
// TabTube: main-process tab manager (one WebContentsView per tab).
import { TabManager, TAB_BAR_HEIGHT } from './tabManager'
import { TAB_BAR_HTML } from './tabBarPage'
...
import { constants as fsConstants, existsSync, writeFileSync } from 'fs'
...
function runApp() {
  ...
  // TabTube: maps a tab-bar chrome view's webContents id → its window's TabManager,
  // so `tabbar:action` messages from the chrome route to the right window.
  const tabManagersByChromeId = new Map()

  // TabTube: find the TabManager that owns a given tab view's webContents.
  const managerForContents = (contents) => {
    for (const manager of tabManagersByChromeId.values()) {
      if (manager.tabs.some(tab => tab.view.webContents === contents)) {
        return manager
      }
    }
    return null
  ...
  async function createWindow(
    ...
    // TabTube: the window-open handler is applied per tab view instead (see the
    // TabManager `configureView` below), so a "new window" gesture (middle-click /
    // Ctrl-click) opens a background TAB rather than a separate window.
    ...
    // TabTube: instead of loading the app directly into the window, host a tab-bar
    // chrome strip plus one WebContentsView per tab. Each tab is an independent
    // renderer, so tabs play independently and never reset one another.

    // TabTube: the window's own webContents is intentionally left unloaded — loading
    // about:blank gives it an opaque origin that crashes production site isolation.
    // The window is shown when the tab-bar chrome paints (see the show hook below).

    const chromeView = new WebContentsView({
      webPreferences: {
        preload: process.env.NODE_ENV === 'development'
          ? path.resolve(__dirname, '../../dist/tabbar.js')
          : path.resolve(__dirname, 'tabbar.js')
      }
    })
    newWindow.contentView.addChildView(chromeView)
    // Write the chrome HTML to a real file and load it (a file:// origin is valid;
    // a data: URL has an opaque origin that Chromium rejects under production site
    // isolation).
    const tabBarFile = path.join(app.getPath('userData'), 'tabtube-tabbar.html')
    writeFileSync(tabBarFile, TAB_BAR_HTML)
    chromeView.webContents.loadFile(tabBarFile)

    const tabManager = new TabManager(newWindow, {
      ...
      configureView: (view) => {
        view.webContents.setWindowOpenHandler((details) => {
          ...
          if (parsed !== null && isFreeTubeUrl(view.webContents.getURL())) {
            if (isFreeTubeUrl(parsed)) {
              // FreeTube link → new BACKGROUND tab (current tab keeps playing).
              tabManager.newTab(details.url, { activate: false })
            } else if (
              parsed.protocol === 'http:' || parsed.protocol === 'https:' ||
              parsed.protocol === 'mailto:' || parsed.protocol === 'tel:' ||
              (parsed.protocol === 'bitcoin:' && parsed.pathname === ABOUT_BITCOIN_ADDRESS)
            ) {
      ...
      onTabsChanged: (tabs) => {
        if (!chromeView.webContents.isDestroyed()) {
          chromeView.webContents.send('tabbar:update', tabs)
        }
      }
    })

    newWindow._tabManager = tabManager
    tabManagersByChromeId.set(chromeView.webContents.id, tabManager)

    const layoutChrome = () => {
      const [width] = newWindow.getContentSize()
      chromeView.setBounds({ x: 0, y: 0, width, height: TAB_BAR_HEIGHT })
    }
    // Defer so getContentSize() reflects the settled bounds after maximize/fullscreen.
    const deferLayoutChrome = () => { setTimeout(layoutChrome, 0) }
    for (const event of ['resize', 'maximize', 'unmaximize', 'enter-full-screen', 'leave-full-screen']) {
      newWindow.on(event, deferLayoutChrome)
    }
    newWindow.on('closed', () => {
      tabManagersByChromeId.delete(chromeView.webContents.id)
    })
    layoutChrome()

    // Open the first tab (a deep-link startup URL, or the app root).
    tabManager.newTab(windowStartupUrl || ROOT_APP_URL)
    ...
    // TabTube: show the window once the tab-bar chrome has painted (the window's own
    // webContents never loads a page in the tabbed layout, so ready-to-show/
    // did-finish-load on it would never fire).
  ...
  // TabTube: tab-bar chrome actions (new / close / activate a tab).
  ipcMain.on('tabbar:action', (event, action) => {
    const manager = tabManagersByChromeId.get(event.sender.id)
    if (!manager || action == null) { return }
    switch (action.type) {
      case 'new':
        manager.newTab(ROOT_APP_URL, { focusSearch: true })
        break
      case 'close':
        manager.closeTab(action.id)
        break
      case 'activate':
  ...
  ipcMain.on(IpcChannels.CREATE_NEW_WINDOW, (event, path, query, searchQueryText) => {
    ...
    // TabTube: "open in new window" now opens a new BACKGROUND tab in the sender's
    // window. Falls back to a real window only if no manager is found.
    const manager = managerForContents(event.sender)
    if (manager) {
      manager.newTab(windowStartupUrl, { activate: false })
      return
    }
  ...
  async function setMenu() {
    ...
    const template = [
      ...
      },
      {
        ...
        submenu: [
          { role: 'minimize' },
          // TabTube: remap window-close to Ctrl/Cmd+Shift+W so the renderer can use
          // Ctrl/Cmd+W to close the active tab (browser convention).
          { role: 'close', accelerator: process.platform === 'darwin' ? 'Cmd+Shift+W' : 'Ctrl+Shift+W' }
        ]
      },
      ...process.platform === 'darwin'
        ? [
