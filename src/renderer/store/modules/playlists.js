...
const actions = {
  async addPlaylist({ state, commit, rootState, dispatch }, payload) {
    ...
    try {
      // TabTube fix: strip Vue reactivity before the IPC boundary. Electron's structured
      // clone can't serialize a reactive Proxy (e.g. the default playlists come straight
      // from Vuex state), which crashed first-run playlist creation. Report upstream.
      await DBPlaylistHandlers.create([JSON.parse(JSON.stringify(payload))])
  ...
  async addPlaylists({ state, commit, rootState, dispatch }, payload) {
    ...
    try {
      // TabTube fix: strip Vue reactivity before the IPC boundary (see addPlaylist).
      await DBPlaylistHandlers.create(JSON.parse(JSON.stringify(payload)))
