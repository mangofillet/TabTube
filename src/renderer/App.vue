...
// TabTube: the built-in update check pings FreeTube's release feed, which would
// misrepresent this fork as official. Disabled until TabTube has its own release feed.
const UPDATE_CHECKS_ENABLED = false

async function checkForNewUpdates() {
  if (!UPDATE_CHECKS_ENABLED || !checkForUpdates.value) {
    return
  }
 ...
 */
/**
 * TabTube: resolve an internal (hash-routed) app link under the event target.
 * @param {PointerEvent} event
 * @returns {{ path: string, query: object }|null}
 */
function internalTabTarget(event) {
  const anchor = event.target?.closest?.('a[href]')
  if (!anchor) { return null }
  const href = anchor.href
  const hashIndex = href.indexOf('#/')
  if (hashIndex === -1) { return null }
  if (!href.startsWith(window.location.origin) && !href.startsWith('file://')) { return null }
...
// TabTube: hand the link to the main process, which opens it as a new background
// tab (WebContentsView) so this tab keeps playing.
function openInBackgroundTab(target) {
  window.ftElectron.openInNewWindow(target.path, target.query, null)
}
...
function handleClick(event) {
  ...
  // Ctrl/Cmd+click an internal link → new background tab.
  if (process.env.IS_ELECTRON && (event.ctrlKey || event.metaKey)) {
    const target = internalTabTarget(event)
    if (target) {
      event.preventDefault()
      openInBackgroundTab(target)
    }
  }
}
...
function handleAuxClick(event) {
  ...
  // only handle the middle button, otherwise the context menu breaks.
  ...
  // Middle-click an internal link → new background tab.
  if (process.env.IS_ELECTRON) {
    const target = internalTabTarget(event)
    if (target) {
      event.preventDefault()
